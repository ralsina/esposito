...
void process_sound(void)
{
  ...
  //convert to 16bit stereo for audio_stream_write
  static int16_t stereo_sample_buf[TMNUM * 2];
  ...
  for (short i = 0; i < TMNUM; i++)
  {
    ...
    stereo_sample_buf[i * 2] = (int16_t)sample;
    stereo_sample_buf[i * 2 + 1] = (int16_t)sample;
  }

  audio_stream_write(stereo_sample_buf, TMNUM);
}
